...
abstract contract OracleController is
    BaseAppInsuranceController,
    BaseOracleListenerHandler,
    BaseAppContract,
    OwnableContract,
    PayableContract
{
    ...
    event FlightStatusInfoRequested(
        uint8 index,
    ...
    event FlightStatusInfoSubmitted(
        address airlineAddress,
        string flightNumber,
        uint256 departureTime,
        uint8 flightStatus
    ...
    function registerOracle() external payable requireIsOperational {
        ...
        uint8[3] memory indexes = generateThreeNonDuplicatedIndexes(
            msg.sender,
            ORACLE_RANDOM_INDEX_CEIL
        );
    ...
    function unregisterOracle() external requireIsOperational {
        require(
            ...
            "Only a registered oracle can be unregistered"
        );
        ...
        payTo(
            ...
            "Cannot pay back the registration fee for oracle"
        );
    }
    ...
    function requestFlightStatusInfo(
        address airlineAddress,
        ...
        bytes32 oracleKey = getOracleKey(
            index,
            airlineAddress,
            flightNumber,
            departureTime
        ...
        FlightStatusInfo
            storage oracleFlightStatusInfo = oracleFlightStatusInfos[oracleKey];

        require(
            !oracleFlightStatusInfo.isRequested,
            "Can not request flight status info with the same index twice"
        );

        require(
            oracleFlightStatusInfo.requester == address(0),
            "The same oracle request to request flight information cannot be done twice"
        );
        ...
        emit FlightStatusInfoRequested(
            index,
            airlineAddress,
            flightNumber,
            departureTime
    ...
    function submitFlightStatusInfo(
        uint8 index,
        address airlineAddress,
        string calldata flightNumber,
        uint256 departureTime,
        ...
        require(
            oracleFlightStatusInfos[oracleKey].isRequested,
            "The submitted flight status information has never been requested"
        );
        require(
            oracleFlightStatusInfos[oracleKey].isOpen,
            "The flight status information request has been closed"
        );

        oracleFlightStatusInfos[oracleKey].responses[statusCode].push(
            msg.sender
        );

        emit FlightStatusInfoSubmitted(
            airlineAddress,
            flightNumber,
            departureTime,
            statusCode
        ...
        if (
            oracleFlightStatusInfos[oracleKey].responses[statusCode].length >=
            ORACLE_RESPONSES_REQUIRED_FOR_VALIDATION
        ) {
            closeFlight(flightKey);
            oracleFlightStatusInfos[oracleKey].isOpen = false;
            emit FlightStatusInfoUpdated(
                airlineAddress,
                flightNumber,
                departureTime,
                statusCode
            );
            processFlightStatusInfoUpdated(
                airlineAddress,
                flightNumber,
                departureTime,
                statusCode
    ...
    function generateThreeNonDuplicatedIndexes(
        address account,
        uint8 randomNumberIndexCeil
    ) private returns (uint8[3] memory) {
    ...
    function getRandomIndex(address account, uint8 randomNumberIndexCeil)
        private
        returns (uint8)
    {
        uint8 random = uint8(
            uint256(
                keccak256(
                    abi.encodePacked(
                        blockhash(block.number - randomNumberGeneratorNonce),
                        randomNumberGeneratorNonce,
                        account
                    )
                )
            ) % randomNumberIndexCeil
        );
        randomNumberGeneratorNonce =
            (randomNumberGeneratorNonce + 1) %
            BLOCKS_USED_FOR_RANDOM_NUMBER_GENERATION;
        return random;
    }
}
