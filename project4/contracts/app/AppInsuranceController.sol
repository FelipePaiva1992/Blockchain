...
abstract contract AppInsuranceController is
    BaseAppInsuranceController,
    BaseAppInsurerController,
    BaseOracleListenerHandler,
    BaseAppContract,
    PayableContract
{
    uint256 internal constant MIN_INSURANCE_PRICE = 1 wei;
    uint256 internal constant MAX_INSURANCE_PRICE = 1 ether;
    ...
    function registerFlight(
        ...
        requireRegisteredAirline(
            airlineAddress,
            "Only an registered airline can register flights"
        );

        bytes32 insuredObjectKey = getFlightKey(
            airlineAddress,
            flightNumber,
            departureTime
        );
        require(
            !flights[insuredObjectKey].isRegistered,
            "The same flight cannot be registered twice"
        );
    ...
    function buyFlightInsurance(
        ...
        suretyDataContract.buyInsurance{value: msg.value}(
            msg.sender,
            flightKey
        );
    }

    function withdrawFlightInsuranceCredit(
        address airlineAddress,
        string memory flightNumber,
        uint256 departureTime
    ) external requireIsOperational {
    ...
    function processFlightStatusInfoUpdated(
        address airlineAddress,
        string memory flightNumber,
        uint256 departureTime,
        uint8 statusCode
    ...
    function getFlightKey(
        ...
        return
            keccak256(
                abi.encodePacked(airlineAddress, flightNumber, departureTime)
            );
    }

    function requireRegisteredFlight(bytes32 flightKey) internal view override {
        require(
    ...
    function requireNotClosedFlight(bytes32 flightKey) internal view override {
        require(
            !flights[flightKey].isClosed,
            "The flight has been already closed"
        );
    }
