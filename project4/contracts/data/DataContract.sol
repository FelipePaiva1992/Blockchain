...
abstract contract DataContract is BaseDataContract, OwnableContract {
    ...
    event FundingReceived(address sponsorAddress, uint256 amountPaid);

    receive() external payable {
        Funding memory funding = Funding(msg.sender, msg.value);

        fundings.push(funding);
        emit FundingReceived(msg.sender, msg.value);
    }

    function authorizeContractCaller(address dataContract)
        external
        override
    ...
    function deauthorizeContractCaller(address dataContract)
        external
        override
        requireContractOwner
    {
