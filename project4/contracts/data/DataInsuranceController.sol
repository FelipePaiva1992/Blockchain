...
abstract contract DataInsuranceController is
    PayableContract,
    ...
    DataContract
{
    ...
    uint256 private minInsurancePrice = 1 wei;
    uint256 private maxInsurancePrice = 1 ether;
    ...
    event InsurancePolicyStateChanged(
    ...
    function registerInsuredObject(bytes32 insuredObjectKey)
        external
        override
    ...
    {
        require(
            !insuredObjects[insuredObjectKey].isRegistered,
            "An insured object cannot be registered twice"
        );
    ...
    {
        require(
            msg.value >= minInsurancePrice && msg.value <= maxInsurancePrice,
            "Invalid insurance price paid"
        );

        InsuredObject storage insuredObject = insuredObjects[insuredObjectKey];
        require(
            insuredObject.isRegistered,
            "The insured object is not registered"
        );
        require(
            insuredObject.insurancePolicies[insureeAddress].state ==
                InsurancePolicyState.AVAILABLE,
            "The same insurance policy cannot be bought twice"
        );
    ...
    function closeAllInsurances(bytes32 insuredObjectKey)
        external
        override
        requireIsOperational
        requireAuthorizedCaller
    {
        ...
        for (uint256 i = 0; i < insuredObject.insureeAddresses.length; i++) {
            address insureeAddress = insuredObject.insureeAddresses[i];
            InsurancePolicy storage insurancePolicy = insuredObject
                .insurancePolicies[insureeAddress];
            require(
                insurancePolicy.state == InsurancePolicyState.ACQUIRED,
                "Insurance cannot be closed or it has been already closed"
            );
        ...
        requireAuthorizedCaller
    {
        ...
        for (uint256 i = 0; i < insuredObject.insureeAddresses.length; i++) {
            ...
            require(
                insurancePolicy.state == InsurancePolicyState.ACQUIRED,
                "Credit retrieval cannot be approved or it has been already approved"
            );
    ...
    function withdrawInsuranceCredit(
        address insureeAddress,
        bytes32 insuredObjectKey
    ) external override requireIsOperational requireAuthorizedCaller {
        InsuredObject storage insuredObject = insuredObjects[insuredObjectKey];
        require(
            insuredObject.isRegistered,
            "The insured object is not registered"
        ...
        require(
            insurancePolicy.state == InsurancePolicyState.CREDIT_APPROVED,
            "Credit retrieval is not approved or it has been already withdrawn"
        );
        require(
            insurancePolicy.amountWithdrawn == 0,
        ...
        payTo(
            insureeAddress,
            insurancePolicy.amountWithdrawn,
            "Insurance policy credit withdrawn failed."
        );
        triggerInsurancePolicyStateChange(insuredObjectKey, insureeAddress);
    }
    ...
    function getInsureePaidAmount() private view returns (uint256) {
    ...
    function triggerInsurancePolicyStateChange(
        bytes32 insuredObjectKey,
        address insureeAddress
    ) private {
        InsuredObject storage insuredObject = insuredObjects[insuredObjectKey];
        emit InsurancePolicyStateChanged(
            insureeAddress,
            insuredObjectKey,
            uint256(insuredObject.insurancePolicies[insureeAddress].state)
        );
    }
}
